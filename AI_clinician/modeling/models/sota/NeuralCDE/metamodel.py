...
class NeuralCDE(torch.nn.Module):
    """A Neural CDE models. Provides a wrapper around the lower-level cdeint function, to get a flexible Neural CDE
    models.
    ...
    ```
    where X is determined by the data, and given some terminal time t_N, then this models first computes z_{t_N}, then
    performs a linear function on it, and then outputs the result.

    It's known that linear functions on CDEs are universal approximators, so this is a very general type of models.
    """
    def __init__(self, func, input_channels, hidden_channels, initial=True):
        """
        Arguments:
            func: As cdeint.
    ...
    def extra_repr(self):
        return "input_channels={}, hidden_channels={}, initial={}" \
               "".format(self.input_channels, self.hidden_channels, self.initial)
    ...
    def forward(self, times, coeffs, final_index, z0=None, stream=False, **kwargs):
        ...
        Arguments:
            ...
            stream: Whether to return the result of the Neural CDE models at all times (True), or just the final time
