...
def correct_features(df, provenance=None, no_gender_imputation=False, no_elixhauser_imputation=False):
    # CORRECT GENDER
    df[C_GENDER] = pd.to_numeric(df[C_GENDER], errors='coerce')
    ...
    # FIX EXTUBATED - make it binary
    if provenance:
        provenance.record("replace NaN extubated", row=df.loc[pd.isna(df[C_EXTUBATED])].index, col=C_EXTUBATED)
    df.loc[pd.isna(df[C_EXTUBATED]), C_EXTUBATED] = 0
    df.loc[df[C_EXTUBATED] > 0, C_EXTUBATED] = 1

    # FIX Elixhauser missing values
    if not no_elixhauser_imputation:
        if provenance:
            provenance.record("replace NaN elixhauser with median", row=df.loc[pd.isna(df[C_ELIXHAUSER])].index, col=C_ELIXHAUSER)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-impute-computed', dest='impute_computed', default=True, action='store_false',
                        help="Don't impute computed features (SOFA, SIRS, etc.) when they can't be computed due to missing values")
    parser.add_argument('--no-gender-imputation', dest='no_gender_imputation', default=False, action='store_true',
                        help="Don't impute missing gender values with mode")
    parser.add_argument('--no-elixhauser-imputation', dest='no_elixhauser_imputation', default=False, action='store_true',
                        help="Don't impute missing Elixhauser values with median")
    ...
    if args.correct_features:
        ...
        df = correct_features(df, provenance=provenance, 
                             ...
                             no_elixhauser_imputation=args.no_elixhauser_imputation)
    ...
    if args.computed_features:
        print("Computing P/F")
        df[C_PAO2_FIO2] = compute_pao2_fio2(df, impute_computed=args.impute_computed)
        print("Computing shock index")
        df[C_SHOCK_INDEX] = compute_shock_index(df, impute_computed=args.impute_computed)
        print("Computing SOFA")
        df[C_SOFA] = compute_sofa(df, timestep_resolution=args.resolution, impute_computed=args.impute_computed)
        print("Computing SIRS")
        df[C_SIRS] = compute_sirs(df, impute_computed=args.impute_computed)
        
        # Only impute computed features if impute_computed is True
        if args.impute_computed:
            print("Imputing computed features with average values")
            # Impute computed features that couldn't be calculated
            for col in [C_PAO2_FIO2, C_SOFA, C_SIRS]:  # Remove C_SHOCK_INDEX since it's handled in compute_shock_index
                if df[col].isna().any():
                    avg_val = df[col].mean()
                    if provenance:
                        provenance.record(f"impute {col} with average", 
                                        row=df[df[col].isna()].index, 
                                        col=col)
                    df.loc[df[col].isna(), col] = avg_val
        else:
            print("Keeping computed features as null when they can't be computed")
    ...
    if args.mask_file:
        ...
        # Ensure mask file directory exists
        mask_dir = os.path.dirname(args.mask_file)
